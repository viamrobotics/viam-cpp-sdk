...
::grpc::Status ModuleService_::AddResource(
  ...
  std::shared_ptr<ModelRegistration> reg =
      Registry::lookup_resource(cfg.api, cfg.model);
  if (reg != nullptr) {
  ...
  if (module->services.find(cfg.api) == module->services.end()) {
    ...
    return grpc::Status(grpc::UNKNOWN,
                        "module cannot service api " + cfg.api.to_string());
  }
...
::grpc::Status ModuleService_::ReconfigureResource(
  ...
  std::shared_ptr<ResourceBase> res =
      sub_svc->resource(cfg.resource_name().name);
  if (res == nullptr) {
    return grpc::Status(grpc::UNKNOWN, "unable to reconfigure resource " +
...
::grpc::Status ModuleService_::ValidateConfig(
  ...
  std::shared_ptr<ModelRegistration> reg =
      Registry::lookup_resource(cfg.api, cfg.model);
  if (reg == nullptr) {
    return grpc::Status(grpc::UNKNOWN, "unable to validate resource " +
                                           cfg.resource_name().name +
  ...
  try {
    ...
    return grpc::Status(grpc::UNKNOWN, "validation failure in resource " +
                                           cfg.name + ": " + err);
  }
  return grpc::Status();
};
...
ModuleService_::~ModuleService_() { this->close(); }
...
void ModuleService_::add_api_from_registry(Subtype api) {
  ...
  std::shared_ptr<ResourceServerBase> server =
      rs->create_resource_server(new_svc);
