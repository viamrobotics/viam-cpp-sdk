...
BOOST_AUTO_TEST_CASE(test_get_properties_service) {
  ...
  *req.mutable_name() = "camera";
  ...
  BOOST_CHECK(expected.intrinsic_parameters ==
              Camera::from_proto(resp.intrinsic_parameters()));
  BOOST_CHECK(expected.distortion_parameters ==
              Camera::from_proto(resp.distortion_parameters()));
}
...
BOOST_AUTO_TEST_CASE(test_do_service) {
  ...
  *req.mutable_name() = "camera";
  ...
  std::shared_ptr<ProtoType> expected_pt =
      expected_map->at(std::string("test"));
