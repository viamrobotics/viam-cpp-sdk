...
void SubtypeService::add(Name name, std::shared_ptr<ResourceBase> resource) {
  ...
  try {
    ...
    BOOST_LOG_TRIVIAL(error)
        << "Error adding resource to subtype service: " << exc.what();
  }
  lock.unlock();
};
...
void SubtypeService::do_remove(Name name) {
  std::string short_name = name.short_name();
  if (resources.find(short_name) == resources.end()) {
    throw "attempted to remove resource " + name.to_string() +
        " but it didn't exist!";
  }
  ...
  for (auto &res : resources) {
    ...
    if (shortcut == get_shortcut_name(key) &&
        short_name != get_shortcut_name(key)) {
      if (short_names.find(shortcut) != short_names.end()) {
        short_names.emplace(shortcut, "");
      } else {
...
void SubtypeService::replace_one(Name name,
                                 std::shared_ptr<ResourceBase> resource) {
  lock.lock();
  try {
    do_remove(name);
    ...
    BOOST_LOG_TRIVIAL(error) << "failed to replace resource "
