...
typedef boost::variant<SDK::axis_angles, SDK::euler_angles,
		       SDK::orientation_vector, SDK::orientation_vector_degrees,
		       SDK::quaternion>
    orientation;
...
proto::Orientation OrientationConfig::to_proto() {
	...
	switch (type) {
		case AxisAngles: {
			...
			SDK::axis_angles a = boost::get<SDK::axis_angles>(
		...
		case OrientationVector: {
			...
			SDK::orientation_vector ov =
			    boost::get<SDK::orientation_vector>(
				orientation_.orientation_);
			ovec.set_x(ov.x);
			ovec.set_y(ov.y);
			ovec.set_z(ov.z);
		...
		case OrientationVectorDegrees: {
			...
			SDK::orientation_vector_degrees ovd =
			    boost::get<SDK::orientation_vector_degrees>(
				orientation_.orientation_);
			ovec.set_x(ovd.x);
			ovec.set_y(ovd.y);
			ovec.set_z(ovd.z);
		...
		case EulerAngles: {
			...
			SDK::euler_angles ea = boost::get<SDK::euler_angles>(
			    orientation_.orientation_);
		...
		case Quaternion: {
			...
			SDK::quaternion q = boost::get<SDK::quaternion>(
			    orientation_.orientation_);
			quat.set_w(q.w);
			quat.set_x(q.x);
			quat.set_y(q.y);
