...
bool MockBoard::get_gpio(const std::string& pin, const AttributeMap&) {
    this->peek_pin = pin;
    return this->peek_get_gpio_ret;
}

double MockBoard::get_pwm_duty_cycle(const std::string& pin, const AttributeMap&) {
    this->peek_pin = pin;
...
uint64_t MockBoard::get_pwm_frequency(const std::string& pin, const AttributeMap&) {
    this->peek_pin = pin;
...
void MockBoard::write_analog(const std::string& pin, int value, const AttributeMap&) {
    this->peek_pin = pin;
...
void MockBoard::stream_ticks(std::vector<std::string> const& digital_interrupt_names,
    ...
    for (const auto& name :digital_interrupt_names ) {
        this->peek_callbacks[name] = *ticks;
    }
}
...
std::vector<GeometryConfig> MockBoard::get_geometries(const sdk::AttributeMap&) {
