...
class BoardClient : public Board {
   ...
   pwm_frequency get_pwm_frequency(std::string pin) override;
   void set_pwm_frequency(std::string pin, uint64_t frequency_hz) override;
   AttributeMap do_command(ERROR TODO) override;
   analog_value read_analog(std::string board_name, std::string analog_reader_name) override;
   digital_value read_digital_interrupt(std::string board_name, std::string digital_interrupt_name) override;
