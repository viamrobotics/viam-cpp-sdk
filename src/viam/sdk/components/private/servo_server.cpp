...
::grpc::Status ServoServer::GetStatus(::grpc::ServerContext* context,
                                      const ::viam::common::v1::GetStatusRequest* request,
                                      ::viam::common::v1::GetStatusResponse* response) noexcept {
    return make_service_helper<Servo>(
        "ServoServer::GetStatus", this, context, request)([&](auto&, auto& servo) {
        const ProtoStruct result = servo->get_status();
        *response->mutable_result() = to_proto(result);
    });
}
