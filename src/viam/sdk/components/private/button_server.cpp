...
::grpc::Status ButtonServer::GetStatus(::grpc::ServerContext* context,
                                       const ::viam::common::v1::GetStatusRequest* request,
                                       ::viam::common::v1::GetStatusResponse* response) noexcept {
    return make_service_helper<Button>(
        "ButtonServer::GetStatus", this, context, request)([&](auto&, auto& button) {
        const ProtoStruct result = button->get_status();
        *response->mutable_result() = to_proto(result);
    });
}
