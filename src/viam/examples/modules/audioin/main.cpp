...
ProtoStruct SineWaveAudioIn::do_command(const ProtoStruct& command) {
    for (const auto& entry : command) {
        VIAM_RESOURCE_LOG(*this, info) << "Command entry " << entry.first;
    }
    return command;
}
...
void SineWaveAudioIn::get_audio(std::string const& codec,
    ...
    VIAM_RESOURCE_LOG(*this, info) << "Generating sine wave: " << frequency_ << "Hz, "
    ...
    for (int chunk_idx = 0; chunk_idx < num_chunks; ++chunk_idx) {
        ...
        if (!chunk_handler(std::move(chunk))) {
            VIAM_RESOURCE_LOG(*this, info) << "Chunk handler returned false, stopping";
            break;
        }
    }

    VIAM_RESOURCE_LOG(*this, info) << "Finished generating sine wave";
}
