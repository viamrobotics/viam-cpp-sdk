...
class Options {
   ...
   public:
    ...
    Options(unsigned int refresh_interval_, boost::optional<DialOptions> dial_options_) {
        ...
        if (!dial_options_) {
            std::cout << "dial options don't exist when creating optdions" << std::endl;
        } else {
            std::cout << "dial options exist at creation no problo" << std::endl;
        }
...
class ViamChannel {
   public:
    ...
    ViamChannel(std::shared_ptr<grpc::Channel> channel_, const char* path_, void* runtime) {
        channel = channel_;
        path = path_;
        closed = false;
