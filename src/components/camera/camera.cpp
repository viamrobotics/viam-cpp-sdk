...
std::shared_ptr<ResourceSubtype> Camera::resource_subtype() {
  ...
  const google::protobuf::ServiceDescriptor *sd = p->FindServiceByName(
      viam::component::camera::v1::CameraService::service_full_name());
  if (sd == nullptr) {
    throw std::runtime_error(
        "Unable to get service descriptor for the camera service");
  }
...
Camera::properties Camera::from_proto(
  ...
  viam::component::camera::v1::DistortionParameters
      distortion_parameters_proto = proto.distortion_parameters();
...
bool operator==(const Camera::point_cloud &lhs,
                const Camera::point_cloud &rhs) {
