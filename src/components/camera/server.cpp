...
::grpc::Status CameraServer::DoCommand(
    ...
    std::shared_ptr<std::unordered_map<std::string, std::shared_ptr<ProtoType>>> result = camera->do_command(request->name(), command);
...
::grpc::Status CameraServer::GetPointCloud(
    ...
    Camera::point_cloud point_cloud = camera->get_point_cloud(name, mime_type);
...
::grpc::Status CameraServer::GetProperties(
    ...
    Camera::properties properties = camera->get_properties(name);

     *response->mutable_distortion_parameters() = properties.distortion_parameters.to_proto();
     *response->mutable_intrinsic_parameters() = properties.intrinsic_parameters.to_proto();
