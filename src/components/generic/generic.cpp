...
GenericSubtype::create_resource_server(std::shared_ptr<SubtypeService> svc) {
  return std::make_shared<GenericServer>(svc);
};

std::shared_ptr<ResourceBase>
GenericSubtype::create_rpc_client(std::string name,
...
std::shared_ptr<ResourceSubtype> Generic::resource_subtype() {
  ...
  const google::protobuf::ServiceDescriptor *sd = p->FindServiceByName(
      viam::component::generic::v1::GenericService::service_full_name());
  if (sd == nullptr) {
    throw std::runtime_error(
        "Unable to get service descriptor for the generic service");
  }
  return std::make_shared<GenericSubtype>(sd);
}

Subtype Generic::subtype() { return Subtype(RDK, COMPONENT, "generic"); }
