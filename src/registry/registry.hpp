...
public:
  ...
  virtual std::shared_ptr<ResourceServerBase>
  create_resource_server(std::shared_ptr<SubtypeService> svc) = 0;
  virtual std::shared_ptr<ResourceBase>
...
public:
  ...
  ModelRegistration(
      ResourceType rt, Subtype subtype, Model model,
      std::function<std::shared_ptr<ResourceBase>(Dependencies, Resource)>
          constructor)
      : subtype(std::move(subtype)), resource_type(std::move(rt)),
        model(std::move(model)), construct_resource(std::move(constructor)),
...
public:
  ...
  static std::shared_ptr<ModelRegistration> lookup_resource(Subtype subtype,
                                                            Model model);
  static void
