...
void Registry::register_resource(std::shared_ptr<ModelRegistration> resource) {
  std::string reg_key =
      resource->subtype.to_string() + "/" + resource->model.to_string();
  if (resources.find(reg_key) != resources.end()) {
    std::string err =
        "Cannot add resource with name " + reg_key + "as it already exists";
...
void Registry::register_subtype(
    Subtype subtype, std::shared_ptr<ResourceSubtype> resource_subtype) {
...
std::shared_ptr<ModelRegistration> Registry::lookup_resource(Subtype subtype,
                                                             Model model) {
  ...
  if (resource->type.type == COMPONENT) {
    try {
      ...
      BOOST_LOG_TRIVIAL(debug)
          << "Unable to get resource name from resource base "
          << resource->name;
    }
...
std::unordered_map<std::string, std::shared_ptr<ModelRegistration>>
    Registry::resources;
std::unordered_map<Subtype, std::shared_ptr<ResourceSubtype>>
    Registry::subtypes;
