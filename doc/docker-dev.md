...
The `conan` image is set up to allow creating and building the `viam-cpp-sdk` conan package.
It ships the `viamconan` remote and a `viam-system-gcc-release` profile which the `default`
profile includes, so a bare `conan create .` in the container uses the image's toolchain of
record. See [conan.md](conan.md) for more on this.
...
[build instructions](BUILDING.md#invoking-cmake-to-generate-a-build-system).
